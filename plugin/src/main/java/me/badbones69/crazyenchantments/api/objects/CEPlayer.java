            ...
            if (kit.canAutoEquipt()) {
                if (item.getType().toString().toLowerCase().contains("helmet")) {
                    if (player.getEquipment().getHelmet() == null || player.getEquipment().getHelmet().getType() == Material.AIR) {
                        ArmorEquipEvent event = new ArmorEquipEvent(player, ArmorEquipEvent.EquipMethod.DRAG, ArmorType.HELMET, new ItemStack(Material.AIR), item);
                    ...
                    if (player.getEquipment().getChestplate() == null || player.getEquipment().getChestplate().getType() == Material.AIR) {
                        ArmorEquipEvent event = new ArmorEquipEvent(player, ArmorEquipEvent.EquipMethod.DRAG, ArmorType.CHESTPLATE, new ItemStack(Material.AIR), item);
                        Bukkit.getPluginManager().callEvent(event);
                        if (!event.isCancelled()) {
                    ...
                    if (player.getEquipment().getLeggings() == null || player.getEquipment().getLeggings().getType() == Material.AIR) {
                        ArmorEquipEvent event = new ArmorEquipEvent(player, ArmorEquipEvent.EquipMethod.DRAG, ArmorType.LEGGINGS, new ItemStack(Material.AIR), item);
                        Bukkit.getPluginManager().callEvent(event);
                        if (!event.isCancelled()) {
                    ...
                    ArmorEquipEvent event = new ArmorEquipEvent(player, ArmorEquipEvent.EquipMethod.DRAG, ArmorType.BOOTS, new ItemStack(Material.AIR), item);
                    Bukkit.getPluginManager().callEvent(event);
                    if (!event.isCancelled()) {
